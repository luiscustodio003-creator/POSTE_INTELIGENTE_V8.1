...
static void _processar_mensagem(char *msg, const char *ip)
{
    ...
    /* STATUS:<id>:<estado> ────────────────────────────────────
       Usado para notificação de estado do radar:
         SAFE → viz. perdeu radar (RADAR_FAIL)
         OK   → viz. recuperou radar (RADAR_OK)
       Determina se veio do viz. esq. ou dir. pelo IP de origem.
    ─────────────────────────────────────────────────────────── */
    if (strncmp(msg, "STATUS:", 7) == 0) {
        int  id    = 0;
        char est[16] = {0};
        if (sscanf(msg + 7, "%d:%15s", &id, est) == 2 && id != POSTE_ID) {
            neighbor_t *v = _encontrar_ou_criar_vizinho(ip, id, -1);
            if (v) {
                v->status    = _str_para_status(est);
                v->last_seen = _agora_ms();
            }
            /* Determina se o remetente é viz. esq. ou dir. */
            neighbor_t *esq = udp_manager_get_neighbor_by_pos(POST_POSITION - 1);
            neighbor_t *dir = udp_manager_get_neighbor_by_pos(POST_POSITION + 1);
            bool from_left  = (esq && strcmp(esq->ip, ip) == 0);
            bool from_right = (dir && strcmp(dir->ip, ip) == 0);

            if (strcmp(est, "SAFE") == 0) {
                /* Viz. perdeu radar */
                if (from_left || from_right) {
                    ESP_LOGW(TAG, "RADAR_FAIL de %s (viz.%s)",
                             ip, from_left ? "esq" : "dir");
                    on_radar_fail_received(from_left);
                }
            } else if (strcmp(est, "OK") == 0) {
                /* Viz. recuperou radar — só reage se tinha FAIL */
                if (from_left || from_right) {
                    ESP_LOGI(TAG, "RADAR_OK de %s (viz.%s)",
                             ip, from_left ? "esq" : "dir");
                    on_radar_ok_received(from_left);
                }
            }
        }
        return;
...
__attribute__((weak))
void on_radar_fail_received(bool from_left) { (void)from_left; }

__attribute__((weak))
void on_radar_ok_received(bool from_left) { (void)from_left; }
